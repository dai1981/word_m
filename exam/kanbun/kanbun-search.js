/* 漢文用語 検索ウィジェット（依存なし・自己完結）
 *
 * 使い方：任意のページに次を置くだけ。
 *   <div id="kanbun-search"></div>
 *   <script src="/exam/kanbun/kanbun-search.js"></script>
 *
 * 見出し・読み・カナ・意味・コアで部分一致検索し、
 * 候補をクリック（またはEnter）で /exam/kanbun/{id}.html へ遷移します。
 * インデックスは /exam/kanbun/kanbun-index.json（generate_kanbun.py が自動生成）。
 */
(function () {
  "use strict";
  if (window.__kanbunSearchInit) return;
  ...
  function injectCSS() {
    if (document.getElementById("kanbun-search-css")) return;
    var css = ''
      ...
      + '.ks-item.on,.ks-item:hover{background:#eef2f8;}'
      + '.ks-main{display:flex;flex-direction:column;min-width:0;flex:1;}'
      + '.ks-mid{font-family:"Hiragino Mincho ProN","Yu Mincho",serif;font-size:17px;font-weight:700;color:#2a3340;line-height:1.25;}'
      + '.ks-core{font-size:12.5px;color:#5f6b78;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}'
  ...
  function build(container) {
    ...
    wrap.innerHTML =
      '<form class="ks-form" action="#" autocomplete="off">' +
        '<input class="ks-input" type="text" placeholder="漢文用語を検索（句法・語・故事成語）" ' +
    ...
    function go(d) { if (d) window.location.href = "/exam/kanbun/" + d.id + ".html"; }

    function render(list) {
      current = list; active = -1;
      if (!list.length) {
        box.innerHTML = '<div class="ks-empty">該当する漢文用語が見つかりません</div>';
      ...
      for (var i = 0; i < list.length; i++) {
        var d = list[i];
        var col = LEVEL_COLOR[d.level] || "#5f6b78";
  ...
  function init() {
    var hosts = document.querySelectorAll("#kanbun-search");
